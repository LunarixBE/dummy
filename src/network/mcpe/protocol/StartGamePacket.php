...
class StartGamePacket extends DataPacket implements ClientboundPacket{
	...
	public static function create(
		...
		$result->serverTelemetryData = new ServerTelemetryData("", "", "", "");
	...
	protected function decodePayload(PacketSerializer $in) : void{
		...
		if($in->getProtocolId() <= ProtocolInfo::PROTOCOL_1_21_50){
			...
			for($i = 0, $count = $in->getUnsignedVarInt(); $i < $count; ++$i){
				...
				$this->itemTable[] = new ItemTypeEntry($stringId, $numericId, $isComponentBased ?? false, -1, new CacheableNbt(new CompoundTag()));
			}
		}
		...
		if($in->getProtocolId() >= ProtocolInfo::PROTOCOL_1_21_100 && $in->getProtocolId() <= ProtocolInfo::PROTOCOL_1_21_124){
	...
	protected function encodePayload(PacketSerializer $out) : void{
		...
		if($out->getProtocolId() >= ProtocolInfo::PROTOCOL_1_21_100 && $out->getProtocolId() <= ProtocolInfo::PROTOCOL_1_21_124){
