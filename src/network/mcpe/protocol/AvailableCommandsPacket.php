...
class AvailableCommandsPacket extends DataPacket implements ClientboundPacket{
	...
	public static function convertArg(int $protocolId, int $type) : int{
		...
		if($protocolId >= ProtocolInfo::PROTOCOL_1_16_100){
			return match ($type) {
				...
				self::ARG_FLAG_VALID | self::ARG_TYPE_VALUE => self::ARG_FLAG_VALID | 0x03,
				self::ARG_FLAG_VALID | self::ARG_TYPE_WILDCARD_INT => self::ARG_FLAG_VALID | 0x04,
				self::ARG_FLAG_VALID | self::ARG_TYPE_OPERATOR => self::ARG_FLAG_VALID | 0x05,
				self::ARG_FLAG_VALID | self::ARG_TYPE_TARGET => self::ARG_FLAG_VALID | 0x06,
				self::ARG_FLAG_VALID | self::ARG_TYPE_WILDCARD_TARGET => self::ARG_FLAG_VALID | 0x07,
				self::ARG_FLAG_VALID | self::ARG_TYPE_FILEPATH => self::ARG_FLAG_VALID | 0x0f,
				self::ARG_FLAG_VALID | self::ARG_TYPE_STRING => self::ARG_FLAG_VALID | 0x1f,
				self::ARG_FLAG_VALID | self::ARG_TYPE_POSITION => self::ARG_FLAG_VALID | 0x28,
				self::ARG_FLAG_VALID | self::ARG_TYPE_MESSAGE => self::ARG_FLAG_VALID | 0x2b,
				self::ARG_FLAG_VALID | self::ARG_TYPE_RAWTEXT => self::ARG_FLAG_VALID | 0x2d,
				self::ARG_FLAG_VALID | self::ARG_TYPE_JSON => self::ARG_FLAG_VALID | 0x31,
				self::ARG_FLAG_VALID | self::ARG_TYPE_COMMAND => self::ARG_FLAG_VALID | 0x38,
				default => $type,
			};
		}
		...
		return match ($type) {
			...
			self::ARG_FLAG_VALID | self::ARG_TYPE_TARGET => self::ARG_FLAG_VALID | 0x06,
			self::ARG_FLAG_VALID | self::ARG_TYPE_WILDCARD_TARGET => self::ARG_FLAG_VALID | 0x07,
			self::ARG_FLAG_VALID | self::ARG_TYPE_FILEPATH => self::ARG_FLAG_VALID | 0x0e,
			self::ARG_FLAG_VALID | self::ARG_TYPE_STRING => self::ARG_FLAG_VALID | 0x1b,
			self::ARG_FLAG_VALID | self::ARG_TYPE_POSITION => self::ARG_FLAG_VALID | 0x1d,
			self::ARG_FLAG_VALID | self::ARG_TYPE_MESSAGE => self::ARG_FLAG_VALID | 0x20,
			self::ARG_FLAG_VALID | self::ARG_TYPE_RAWTEXT => self::ARG_FLAG_VALID | 0x22,
			self::ARG_FLAG_VALID | self::ARG_TYPE_JSON => self::ARG_FLAG_VALID | 0x25,
			self::ARG_FLAG_VALID | self::ARG_TYPE_COMMAND => self::ARG_FLAG_VALID | 0x2c,
			default => $type,
		};
	}
			...
			for($i = 0, $count = $in->getUnsignedVarInt(); $i < $count; ++$i){
				...
				for($j = 0, $valueCount = $in->getUnsignedVarInt(); $j < $valueCount; ++$j){
					if($in->getProtocolId() >= ProtocolInfo::PROTOCOL_1_21_130){
						$valueName = $chainedSubcommandValueNames[$in->getUnsignedVarInt()];
						$valueType = $in->getUnsignedVarInt();
					}else{
	...
	protected function getCommandData(array $enums, array $postfixes, array $allChainedSubCommandData, PacketSerializer $in) : CommandData{
		...
		$permission = $in->getProtocolId() >= ProtocolInfo::PROTOCOL_1_21_130 ? CommandPermissions::fromName($in->getString()) : $in->getByte();
		$aliases = $enums[$in->getLInt()] ?? null;

		$chainedSubCommandData = [];
		if($in->getProtocolId() >= ProtocolInfo::PROTOCOL_1_20_10){
			for($i = 0, $count = $in->getUnsignedVarInt(); $i < $count; ++$i){
				$index = $in->getProtocolId() >= ProtocolInfo::PROTOCOL_1_21_130 ? $in->getLInt() : $in->getLShort();
